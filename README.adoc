...
== TestBalloon – A coroutines-first Kotlin Multiplatform test framework
...
You can configure your tests, test suites, and global settings through a unified, small-surface API (the `TestConfig` builder). You can extend the API with custom configurations.
...
If you declare a subclass of `TestSession`, its `testConfig` parameter will define the global configuration for the entire compilation module. This example extends the framework's default configuration:
...
* [ ] Investigate spurious Main dispatcher concurrency conflict
* [x] Rename `TestElement` properties to `test*`, reducing scope pollution? What about `fixture`?
