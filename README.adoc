...
* Remove `isFocused`: It is not really needed and has the bug:
** The comment says: // Disable all non-focused child elements (disabling does not propagate to transitive child elements).
** Disabled detection in Reports relies on the invariant that disabling an element also disables all of its child elements, transitively.
